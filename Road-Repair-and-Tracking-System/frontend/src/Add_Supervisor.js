  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/add_user', {
        ...
        body: JSON.stringify({ UserID: UserID, password: password,name:name,phoneno:phoneno,suburb:suburb,city:user.city,position:"supervisor"})
      });  
      const data = await response.json();   
      if (response.ok) {
        Swal.fire({
            title: "Added!",
            text: "Supervisor successfully added.",
            icon: "success"
          });
        console.log(data);
        history.push({
